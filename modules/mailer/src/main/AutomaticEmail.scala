...
The HungKings team"""
  ...
  If you already have an account linked to this email, you can try to log in with it, or reset your password if you forgot it."""
        Mailer.Message(
          to = email,
          subject = "Could not verify your new HungKings account",
          text = text,
  ...
  def alreadyConfirmed(email: EmailAddress): Funit =
    ...
    mailer.canSend.so:
      mailer.sendOrSkip:
        val text =
          """Your new HungKings account has already been verified, so you can log in and start playing right away."""
        Mailer.Message(
          to = email,
      ...
      lightUser.preloadMany(opponents.flatMap(_.opponentId)) >>
        userWithEmail.emails.current
          ...
          .so: email =>
            ...
            val hello =
              "Hello and thank you for playing correspondence chess on HungKings!"
